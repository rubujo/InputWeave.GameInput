...
InputWeave 目前支援一般 .NET Framework 與 .NET Windows 應用程式。`net8.0` 與 `net10.0` 已實際跑過 `dotnet publish -p:PublishAot=true` 端對端驗證（獨立探測專案 + 實體硬體，詳見 README「支援範圍」段落）；不宣告 single-file 發佈相容性。
