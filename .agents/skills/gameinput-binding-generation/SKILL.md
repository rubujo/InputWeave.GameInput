...
3. 使用 `pwsh ./eng/Update-GameInputVersion.ps1 -Version <版本>`，或直接執行產生器並搭配 `--docs eng/gameinput-xml-docs.json` 與 `--interop-output-dir src/InputWeave.GameInput/Interop/Generated`，重產 `Interop/Generated` 下的列舉、常數、HRESULT、IID、回呼委派、結構配置與 `gameinput-abi-manifest.json`。COM 介面由 `net48`、`net8.0` 與 `net10.0` 共用同一份裸 vtable 投影（見下方「裸 vtable 投影」）。
